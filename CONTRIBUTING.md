...
conceivably run without a UI, it belongs in `Sources/` with a unit test next to it in
`Tests/`. A good smell test: *could the CLI use this?* Then it goes in the package.
...
- `envhub` — the CLI; a thin argument-parsing shell over `Core`.
...
- New or changed `Core`/concern-module logic needs unit tests (Swift Testing —
  `@Test`, one suite per module in `Tests/`).
- `swift test` and the `xcodebuild` build above must both pass.
